...
def garmin_sync_endpoint():
    """API endpoint to trigger Garmin activity sync."""
    try:
        from src.neighbourhood_run import garmin_sync, tracks

        print("--- Garmin Sync ---")
        relevant = garmin_sync.run_full_sync()
        parsed = tracks.parse_all_tracks()

        # Rebuild the map with tracks
        web.create_network_map()

        message = (
            f"Garmin sync complete. "
            f"Relevant activities: {len(relevant)}. "
            f"Parsed tracks: {len(parsed)}."
        )
        print(f"--- {message} ---")
        return jsonify({"status": "success", "message": message})
